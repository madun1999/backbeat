...
def getMeasures(file_path, beats_per_measure, confidence_threshold):
    beat_arr = get_beat_locations_from_wav_file(file_path)
    total_length_sec = get_length_of_wav_file_seconds(file_path)
    audio = get_audio_from_wav_path(file_path)
    measure_arr = []
    ...
    while (index < (len(measure_arr) - 1)):
        chunk_length = 1
        chunk_BPM = get_bpm_from_audio_array(
        ...
        if (index + chunk_length == len(measure_arr) - 1):
            ...
            chunk_BPM = get_bpm_from_audio_array(audio, 0, measure_arr[index], measure_arr[index + chunk_length], total_length_sec)
            chunk_arr.append([chunk_BPM, chunk_length])
            num_chunks += 1
            break
        while (chunk_BPM == 0 and index + chunk_length < (len(measure_arr) - 1)):
            chunk_length += 1
            chunk_BPM = get_bpm_from_audio_array(audio, confidence_threshold, measure_arr[index], measure_arr[index + chunk_length], total_length_sec)
